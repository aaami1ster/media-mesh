...
describe('Resilience Patterns (Integration)', () => {
  ...
  describe('Circuit Breaker', () => {
    it('should check circuit breaker before making request', async () => {
      ...
      await proxyService.proxyToDiscovery('GET', '/programs');

      expect(circuitBreakerService.canExecute).toHaveBeenCalledWith('discovery-service');
    });
    ...
    it('should reject request when circuit is open', async () => {
      ...
      await expect(proxyService.proxyToDiscovery('GET', '/programs')).rejects.toThrow(
        HttpException,
      );
    ...
    it('should record success after successful request', async () => {
      ...
      expect(circuitBreakerService.recordSuccess).toHaveBeenCalledWith('discovery-service');
    });
    ...
    it('should record failure after failed request', async () => {
      ...
      expect(circuitBreakerService.recordFailure).toHaveBeenCalledWith('discovery-service');
    });
  });
